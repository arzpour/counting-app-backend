import { Document, Schema } from "mongoose";
import { ITransaction } from "../types/transaction";

export interface ITransactionDoc extends Document, Omit<ITransaction, "_id"> {}

const transactionSchema = new Schema<ITransactionDoc>(
  {
    amount: { type: Number, required: true },
    transactionDate: { type: String, required: true },
    type: { type: String, required: true },
    reason: { type: String },
    paymentMethod: { type: String, required: true },
    personId: { type: String },
    secondPersonId: { type: String },
    secondDealId: { type: String },
    dealId: { type: String },
    vin: { type: String },
    secondVin: { type: String },
    bussinessAccountId: { type: String },
    description: { type: String },
    providerPersonId: { type: String },
    brokerPersonId: { type: String },
    partnerPersonId: { type: String },
    partnershipProfitSharePercentage: { type: String },
    partnerShipProfit: { type: String },
    isBetweenTwoPerson: { type: Boolean },
    pairGroupId: { type: String },
    profitState: { type: String },
    role: { type: String },
  },
  {
    timestamps: true,
  },
);

export function getTransactionModel(db: any) {
  if (!db) return null;
  if (db.models?.Transaction) return db.models.Transaction;
  return db.model("Transaction", transactionSchema);
}

// const Transaction = mongoose.model<ITransactionDoc>(
//   "Transaction",
//   transactionSchema,
//   "transactions",
// );
// export default Transaction;
